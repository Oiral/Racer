...
public class HoverCarBaseScript : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position, -transform.up, out hit, 10, layerMask))
        {
            ...
            carBase.transform.position = hit.point;
            carBase.transform.rotation = Quaternion.LookRotation(transform.forward, hit.normal);
    ...
    private void FixedUpdate()
    {
        ...
        if (currTurn != 0)
        {
            rb.AddRelativeTorque(transform.up * currTurn * turnStrength);
